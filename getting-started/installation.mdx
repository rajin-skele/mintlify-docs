---
title: "Installation"
description: "Installation and environment checklist for the Elementor utility importer."
---

## Installation Methods

Choose the option that fits your deployment workflow.

### Method 1: WordPress Admin Upload

<Steps>
  <Step title="Get the plugin ZIP">
    Download the ZIP file from your team's release bundle or distribution channel.
  </Step>
  <Step title="Open plugin upload">
    In WordPress admin, go to **Plugins -> Add New** and click **Upload Plugin**.
  </Step>
  <Step title="Install the package">
    Select the ZIP file and click **Install Now**.
  </Step>
  <Step title="Activate the plugin">
    Activate the plugin after WordPress finishes installing it.
  </Step>
</Steps>

### Method 2: FTP Upload

<Steps>
  <Step title="Extract the ZIP">
    Extract the plugin archive on your local machine.
  </Step>
  <Step title="Connect to the server">
    Use your preferred FTP or SFTP client to connect to the WordPress host.
  </Step>
  <Step title="Upload the plugin folder">
    Upload the extracted plugin directory to `/wp-content/plugins/`.
  </Step>
  <Step title="Activate in WordPress">
    Open **Plugins** in WordPress admin and activate the importer plugin.
  </Step>
</Steps>

### Method 3: WP-CLI

If you manage the site with WP-CLI, install from a local ZIP package:

```bash
wp plugin install /path/to/importer.zip --activate
```

## System Requirements

### Minimum Requirements

| Requirement | Minimum | Notes |
|------------|---------|-------|
| **WordPress** | 5.8+ | Multisite supported |
| **PHP** | 7.4+ | 128MB memory recommended |
| **Elementor** | v4 with Atomic Elements | Variables optional but recommended |
| **MySQL** | 5.6+ | MariaDB 10.1+ also supported |

### Recommended Requirements

- **WordPress:** 6.0 or higher
- **PHP:** 8.0 or higher
- **Memory:** 256MB or higher
- **Elementor:** Latest v4 build with Atomic Elements enabled
- **Variables:** Enabled if you plan to import design tokens

## Pre-Installation Checklist

Before installing the importer, verify these points:

<AccordionGroup>
  <Accordion icon="check-circle" title="WordPress version">
    Confirm the site is running a supported WordPress version from **Dashboard -> Updates** or **Tools -> Site Health**.
  </Accordion>
  <Accordion icon="puzzle-piece" title="Elementor v4 setup">
    Make sure Elementor v4 is active and Atomic Elements is enabled.
    <Steps>
      <Step title="Check version">
        Review the installed Elementor version in **Elementor -> System Info**.
      </Step>
      <Step title="Enable Atomic Elements">
        Turn on Atomic Elements in Elementor's settings or experiments screen.
      </Step>
      <Step title="Enable Variables if needed">
        Turn on Variables if your import workflow includes reusable design tokens.
      </Step>
    </Steps>
  </Accordion>
  <Accordion icon="server" title="Server readiness">
    Confirm PHP version, available memory, and file permissions are sufficient for plugin installation and larger imports.
  </Accordion>
</AccordionGroup>

## Post-Installation Setup

### 1. Verify Installation

<Steps>
  <Step title="Confirm plugin status">
    Check **Plugins** in WordPress admin and make sure the importer is active.
  </Step>
  <Step title="Open the importer screen">
    Find the importer admin screen and confirm the CSS import interface loads.
  </Step>
  <Step title="Check Elementor compatibility">
    Verify Elementor still loads normally and the v4 atomic editor is available.
  </Step>
</Steps>

### 2. Import the Starter Framework

<Steps>
  <Step title="Copy the bundled CSS">
    Open the bundled utility stylesheet included with the plugin package.
  </Step>
  <Step title="Paste it into the importer">
    Paste the stylesheet into the importer textarea and run the import.
  </Step>
  <Step title="Verify imported classes">
    Confirm the imported classes appear in Elementor's class management UI.
  </Step>
</Steps>

### 3. Test in Elementor

<Steps>
  <Step title="Open Elementor editor">
    Edit a page or post with Elementor.
  </Step>
  <Step title="Insert an atomic element">
    Add a Heading, Text, or Div Block to the canvas.
  </Step>
  <Step title="Apply a utility class">
    Assign a simple class like `text-2xl` or `flex` to verify the import worked.
  </Step>
  <Step title="Confirm rendering">
    Make sure the styling appears correctly in the editor preview.
  </Step>
</Steps>

## Troubleshooting Installation

<AccordionGroup>
  <Accordion icon="exclamation-triangle" title="Plugin activation failed">
    Common causes include low PHP memory, permission issues, or a damaged upload package. Re-upload the ZIP and review server logs if activation fails again.
  </Accordion>
  <Accordion icon="puzzle-piece" title="Elementor not compatible">
    If the importer warns about Elementor, update to a v4 build and confirm Atomic Elements is enabled before retrying.
  </Accordion>
  <Accordion icon="server" title="Server errors">
    Review PHP memory limits, filesystem permissions, and any web server error logs if imports or activation trigger 500 errors.
  </Accordion>
</AccordionGroup>

### Getting Help

<CardGroup cols={2}>
  <Card title="Common Errors" icon="exclamation-triangle" href="/troubleshooting/common-errors">
    Troubleshoot import failures and class conflicts.
  </Card>
  <Card title="Support Workflow" icon="life-ring" href="/troubleshooting/support">
    See what information to gather before escalating an issue.
  </Card>
  <Card title="FAQ" icon="question-circle" href="/troubleshooting/faq">
    Review the most common setup and usage questions.
  </Card>
  <Card title="Quickstart" icon="rocket" href="/quickstart">
    Move straight into your first working import.
  </Card>
</CardGroup>

## Next Steps

<Check>
  **Installation complete:** Continue with the [quickstart guide](/quickstart) to run your first import.
</Check>

<CardGroup cols={2}>
  <Card title="Start with Quickstart" icon="rocket" href="/quickstart">
    Import your first utility classes.
  </Card>
  <Card title="Explore the Framework" icon="book-open" href="/overview">
    Review the utility surface area before importing larger class sets.
  </Card>
</CardGroup>
