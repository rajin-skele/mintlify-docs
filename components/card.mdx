---
title: "Card Component"
description: "Flexible card component for content containers"
---

## Overview

Flexible card component for content containers. Cards provide a consistent way to display grouped content with proper spacing and styling.

## Card Base

The base card component provides a consistent container for content:

```css
.card {
  background-color: #ffffff;
  border-radius: 8px;
  padding: 24px;
}
```

## Examples

### Basic Card

```html
<div class="card">
  <h3 class="text-xl font-semibold mb-4">
    Card Title
  </h3>
  <p class="text-gray">
    Card content goes here.
  </p>
</div>
```

### Bordered Card

```html
<div class="card border-1 border-solid border-gray-light">
  <h3 class="text-xl font-semibold mb-4">
    Bordered Card
  </h3>
  <p class="text-gray">
    Card with border
  </p>
</div>
```

### Card with Button

```html
<div class="card p-6">
  <h3 class="text-xl font-semibold mb-4">
    Card with Button
  </h3>
  <p class="text-gray mb-4">
    Card content with action button
  </p>
  <button class="btn btn-blue">
    Action
  </button>
</div>
```

## Common Patterns

### Card Grid

```html
<div class="flex flex-wrap gap-6">
  <div class="w-1-3">
    <div class="card p-6">
      <h3 class="text-xl font-semibold mb-4">
        Card Title
      </h3>
      <p class="text-gray mb-4">
        Card content
      </p>
      <button class="btn btn-blue">
        Learn More
      </button>
    </div>
  </div>
  <div class="w-1-3">
    <div class="card p-6">
      <h3 class="text-xl font-semibold mb-4">
        Card Title
      </h3>
      <p class="text-gray mb-4">
        Card content
      </p>
      <button class="btn btn-blue">
        Learn More
      </button>
    </div>
  </div>
  <div class="w-1-3">
    <div class="card p-6">
      <h3 class="text-xl font-semibold mb-4">
        Card Title
      </h3>
      <p class="text-gray mb-4">
        Card content
      </p>
      <button class="btn btn-blue">
        Learn More
      </button>
    </div>
  </div>
</div>
```

### Card with Accent Border

```html
<div class="card p-6" style="border-left: 4px solid #3b82f6;">
  <h3 class="text-xl font-semibold mb-2">
    Card Title
  </h3>
  <p class="text-gray">
    Card content with blue accent border
  </p>
</div>
```

### Card with Image

```html
<div class="card p-0 overflow-hidden">
  <img alt="Card image" class="w-full object-cover" src="image.jpg" style="height: 12rem;"/>
  <div class="p-6">
    <h3 class="text-xl font-semibold mb-4">
      Card Title
    </h3>
    <p class="text-gray">
      Card content with image
    </p>
  </div>
</div>
```

## Best Practices

- **Consistent padding:** Use consistent padding values (p-4, p-6) for card content
- **Clear hierarchy:** Use headings and spacing to create visual hierarchy
- **Combine with utilities:** Use explicit border, background, and spacing utilities to customize cards
- **Responsive design:** Use flexbox and width utilities for responsive card grids

## Related Utilities

<CardGroup cols={2}>
  <Card title="Button" icon="hand-pointer" href="/components/button">
    Button component
  </Card>
  <Card title="Container" icon="box-isometric" href="/components/container">
    Container component
  </Card>
  <Card title="Borders" icon="square" href="/overview/borders">
    Border utilities
  </Card>
  <Card title="Components Overview" icon="puzzle-piece" href="/overview/components">
    Complete components overview
  </Card>
</CardGroup>
